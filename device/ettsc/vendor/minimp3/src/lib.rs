...
impl<R> Decoder<R> {
    ...
    fn decode_frame(&mut self) -> Result<Frame, Error> {
        ...
        // VecDeque may wrap across two slices. The C decoder requires a single
        // contiguous input; keep this borrow alive throughout the FFI call.
        let input = self.buffer.make_contiguous();
        ...
        // Remove exactly the bytes consumed, retaining any incomplete frame.
        self.buffer.drain(..frame_info.frame_bytes as usize);
