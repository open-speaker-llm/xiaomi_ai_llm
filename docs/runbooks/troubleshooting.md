...
先区分故障发生在连接、原生识别、路由还是播放，不要同时重装所有组件。准备好当前系统、组件状态和对应时间段的日志；[日常操作](operations.md)提供读取命令，[当前状态](../status.md)说明已有验证的边界。

| 症状 | 从这里检查 |
|---|---|
| SSH 连接被拒绝，或启动系统变化 | [连接与分区](#ssh-连接被拒绝或启动分区改变) |
| 客户端没有进入待机 | [启动后没反应](#2-启动后没反应) |
| 能唤醒，却不能执行原生命令 | [有“欸”但没有动作](#3-有欸但没有动作) |
| 说话被截断，或启用判停后不回答 | [首轮收音与提交](#首轮提前截断或启用判停后没有回答) |
| 原生问题错误转 LLM | [路由判断](#4-原生成功却转了-llm) |
| 转 LLM 前漏出失败提示 | [失败播报拦截](#5-原生不支持时听到还在学习中) |
| 原生短播报晚到 | [控制播报延迟](#6-原生控制播报延迟) |
| 回答忽大忽小 | [音量](#7-llm-音量忽大忽小) |
| 回答后追问失败或有提示音 | [追问](#8-追问失败或续听时响起欸) |
| 有文本却不出声、EdgeTTS 报错 | [TTS 路线](#9-tts-路线排障) |
