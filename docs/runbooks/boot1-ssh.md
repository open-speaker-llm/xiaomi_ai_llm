...
> 2026-09-05 实机维护补充：本轮开始时，实际 system1（1.76.54）没有 `sshen`，22 端口拒绝连接；本文原有“已成功注入”的记录不能代替设备当日核验。本轮基于重新读取的实机备份补齐了 SSH，并为两套系统加入原生 OTA 拦截。当前维护方案、构建与验证工具见 [双系统 SSH 与受控升级](owner-maintenance.md)。下文旧镜像哈希仅属于历史过程，不应拿来校验本轮镜像。
