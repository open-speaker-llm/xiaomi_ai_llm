...
2026-09-06 原生追问：boot1 已部署 native_asr 原生实时识别，Mac ASR 停止期间有声上下文追问成功，重启自动加载通过；“欸”声修复已部署并通过设备隔离测试，听觉复验待确认。见 [正式集成记录](../history/2026-09-06-boot1-native-followup.md)。

2026-09-06 后续：2026-09-06 已在 S12A 的 boot1/system1（ROM 1.76.54）实测：匹配到的小爱失败提示可被拦截并转 LLM，修正版重启后用户确认正常转接、没有先播失败提示。快速拦截器与客户端已持久安装并验证自动启动，详见 [失败提示修复](../history/2026-09-06-boot1-fallback-guard.md)。
...
6. 执行验证脚本，再检查原生语音服务、客户端自启动、失败提示 guard 与实际音箱功能。若需 boot1 原生追问，重新核验 native_asr 的固件 ABI 和启动脚本摘要；不匹配时先适配，不能只修改哈希绕过检查。最后完成真实追问、静默退出和正常唤醒报时测试。
