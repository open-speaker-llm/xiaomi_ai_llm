...
本页负责 SSH、OTA 和镜像维护；部署语音助手从[上手路线](../getting-started/bringup.md)进入。先确认实际根分区与备份，不把某次记录中的“默认 boot0”或“最终保持 boot1”当作此刻设备状态。

SSH 与升级控制的原始证据见 [2026-09-05 恢复记录](../history/2026-09-05-ssh-ota-recovery.md)，system1 自启动补齐见 [2026-09-06 记录](../history/2026-09-06-boot1-autostart.md)。助手、追问和判停的验收统一看[当前状态](../status.md)。
...
6. 已安装本地判停包时，先按[整包维护说明](../../device/native_endpoint/README.md)核对组合库和版本基线，不能用基础追问安装器覆盖。执行验证脚本，再检查原生语音服务、客户端自启动、失败提示 guard 与实际音箱功能。若需 boot1 原生追问，重新核验 native_asr 的固件 ABI 和启动脚本摘要；不匹配时先适配，不能只修改哈希绕过检查。最后完成真实追问、静默退出和正常唤醒报时测试。
