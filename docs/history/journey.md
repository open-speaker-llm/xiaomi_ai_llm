...
阅读提示：按阶段讲述从设备入口到原生优先、连续追问与首轮判停的演变；当前能力和验收范围以[状态页](../status.md)为准。
...
## 阶段 9：把“能接上”变成“能一起工作”（2026-09-07 至 09-10）

追问接通后，问题开始出现在组件交接处：旧会话的结束指令会影响下一轮，清理音频时可能顺手丢掉会话音量，LLM 的声音也会被麦克风再次识别。于是工作重心从寻找接口转向明确每轮请求的身份、清理顺序和播放所有权。过程见[可靠性总结](2026-09-07-dialog-reliability.md)。

并行唤醒又揭示了另一个层次：回调已经透传，不表示声学回声消除真的拿到了播放参考。确认参考通道必须在采集启动前准备，才有了后来的[并行家居实测](2026-09-09-parallel-wake-reference.md)。随后补齐独立播放器的 [Dirac 初始化](2026-09-10-dirac-playback.md)，并核验 [DeepSeek Flash 的实际问答与播放](2026-09-10-deepseek-flash.md)。这些改进各有验证范围，没有合并成一个笼统的“体验已全部解决”。

## 阶段 10：从等待 final，到真正等用户说完（2026-09-17 至 09-20）

首轮截断最初看起来像客户端太着急：partial 还在变化，就已触发 LLM。改成只读 final 后仍然丢尾句，才确认第二层问题——云端收音本身可能在停顿时已经结束，客户端再等也找不回没有上传的声音。

后续从真实 PCM、环境声对照推进到 Silero VAD，再把结束候选与同一轮原生请求关联。部署过程中还遇到旧内核取时和短时音频积压：模型能跑不代表实时输入总能跟上，独立回放通过也不代表现场通过。失败、回滚和修正都保留在[专题证据](first-turn-endpoint/README.md)。

最终的日常包能够保留约 1.5 秒停顿后的轻声补充，异常残句被拒绝，随后又验证了整机重启与用户断电冷启动后的自动恢复。故事走到了“可以日常使用、继续验证”的阶段；全天稳定性和历史播放问题仍各自保留为待办，见[最终验收](first-turn-endpoint/native-daily-20260919.md)。
